...
const userInterface = (() => {
	...
	const changeGameType = (newGameType) => {
		...
		if (!newGameType) {
			gameType.textContent =
				currentGameType === "Player vs. Player"
					? "Player vs. AI"
					: "Player vs. Player";
		} else {
	...
	document
		.querySelector("#change-game-type")
...
const aiPlayer = (() => {
	...
	const bestMove = (board) => {
		...
		moves.forEach((move) => {
			...
			const score = miniMax(newBoard, 1, false);
			...
			movesAndScores.push({move, score}); // debug
		});
...
const gameBoard = (() => {
	...
	const evaluateWinner = (currentBoard = getBoard()) => {
		// Horizontal wins
		for (let i = 0; i < currentBoard[0].length; i += 1) {
			if (
				currentBoard[i].every(
					(val) => val === currentBoard[i][0] && currentBoard[i][0]
				)
			) {
		...
		const verticalWin = currentBoard[0].find(
			(item, i) =>
				item && item === currentBoard[1][i] && item === currentBoard[2][i]
		);
		...
		for (
			let i = 0, j = currentBoard[0].length;
		...
		if (
			rightDiagonalResults.every(
				(val) =>
					val === rightDiagonalResults[0] && rightDiagonalResults[0] !== ""
			)
		) {
